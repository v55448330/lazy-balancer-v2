...
**MFA (v2.1.8)**: TOTP two-step login (Google/Microsoft Authenticator). Self-service binding in Settings → 基础设置 → MFA card (QR + 10 one-time recovery codes, stored as SHA-256). Disable requires password + valid code. Global toggles (default off): write-operation step-up (60s window, 428 + global retry) and verification-failure lockout (5 fails → 10 min). Cluster slave login tickets require admin MFA enabled; users' MFA state syncs via cluster snapshot and full config backup. Admin can reset any user's MFA (audited). Account lockout (always on): five consecutive wrong login passwords lock the account for 10 minutes (429, timing-equalized against enumeration); password-confirmation endpoints (MFA recovery codes, self password change, privileged API-key creation) share the MFA gate's 10-attempt/10-minute cooldown.
