...
**MFA (v2.1.8)**: TOTP two-step login (Google/Microsoft Authenticator). Self-service binding in Settings → 基础设置 → MFA card (QR + 10 one-time recovery codes, stored as SHA-256). Disable requires password + valid code. Global toggles (default off): write-operation step-up (10-min window, 428 + global retry) and verification-failure lockout (5 fails → 10 min). Cluster slave login tickets require admin MFA enabled; users' MFA state syncs via cluster snapshot and full config backup. Admin can reset any user's MFA (audited).
