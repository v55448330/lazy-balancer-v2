...
- **负载均衡规则**：HTTP/HTTPS 反向代理与 TCP 四层代理；加权轮询、最少连接、IP 哈希、Cookie 粘滞；主动/被动健康检查（HTTP URI、TCP 端口、rise/fall 阈值）
- **免费证书**：Let's Encrypt / ZeroSSL ACME 自动签发（DNS-01，支持 DNSPod、腾讯云），到期自动续签、限流退避重试、手动证书上传
- **主从集群**：从节点注册审批、定期同步规则/证书/用户/密钥/设置、状态上报、一键提升为主节点；从节点全站只读
- **操作日志**：独立审计库，全操作中文留痕（创建/更新/启停/签发/同步/登录等），保留期可配
- **配置备份**：一键导出/导入全量配置（校验后导入），兼容 **Lazy Balancer v1（nginx 版）** 备份的规则迁移
- **监控面板**：实时流量、连接统计、上游健康、规则级指标（HTTP 状态码/流量，TCP 连接数）
- **多用户与 API 密钥**：管理员/只读用户、自管理 API Key（SHA-256）、RESTful v1 API + OpenAPI 文档
...
docker run -d \
  ...
  v55448330/lazy-balancer-v2:v2.0.2
```
