...
const actionTagType = (action: string) => {
  if (action === '创建' || action === '启用' || action === '签发成功' || action === '测试成功' || action === '登录成功') return 'success'
  if (action === '删除' || action === '禁用' || action === '停止' || action === '重启' || action === '签发失败' || action === '测试失败' || action === '登录失败' || action === '同步失败' || action === '重载失败' || action === '写入失败' || action === '恢复失败') return 'danger'
  if (action === '更新' || action === '修改状态' || action === '重置密码' || action === '重载' || action === '续签' || action === '重试' || action === 'CA限流') return 'warning'
  return 'info'
}
