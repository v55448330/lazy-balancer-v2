<template>
  <div class="p-6 max-w-7xl mx-auto">
    <h2 class="text-xl font-semibold text-gray-800 mb-6">规则集</h2>
    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="CRS 规则浏览" name="rules">
        ...
        <el-table :data="rules" v-loading="loadingRules" stripe @row-click="openRuleContent" style="cursor: pointer">
      ...
      <el-tab-pane label="CRS 配置" name="setup">
        <el-card shadow="never" v-loading="loadingSetup">
          <template #header>
            <div class="flex items-center justify-between">
              <span class="font-medium">crs-setup.conf</span>
              <el-button link type="primary" size="small" @click="fetchSetup">刷新</el-button>
            </div>
          </template>
    ...
    <el-dialog v-model="contentDialogVisible" :title="currentFilename" width="900px" top="5vh">
      <div v-loading="loadingContent">
        <pre class="crs-content">{{ currentContent }}</pre>
      </div>
    </el-dialog>
  </div>
</template>
...
const fetchRules = async () => {
  ...
  try {
    ...
    const res = await request.get<APIResponse<{ rules: CRSRuleFile[]; total: number }>>(`/security/crs/rules?${params}`)
    rules.value = res.data?.rules || []
    total.value = res.data?.total || 0
  } catch { rules.value = [] } finally { loadingRules.value = false }
}

const fetchSetup = async () => {
  loadingSetup.value = true
  try {
    const res = await request.get<APIResponse<{ content: string }>>('/security/crs/setup')
    setupContent.value = res.data?.content || '# 文件不存在'
  } catch { setupContent.value = '# 加载失败' } finally { loadingSetup.value = false }
}
...
const openRuleContent = async (row: CRSRuleFile) => {
  ...
  } catch { currentContent.value = '加载失败' } finally { loadingContent.value = false }
}
