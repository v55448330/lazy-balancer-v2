...
| 组 | 内容（按 CRS 文件族） | 建议 |
|---|---|---|
| 920/921/922 | 协议异常 / 协议攻击（请求走私/响应拆分/HPP/头注入）/ multipart 攻击 | 建议启用 |
| 930/931/932 | 路径穿越 LFI / 远程文件包含 RFI / **远程代码执行 RCE** | Web 站点建议（932 属高危面，含命令注入） |
| 933/934 | PHP 攻击 / 通用攻击 | 按技术栈 |
| 941/942/943 | XSS 跨站脚本 / SQL 注入 / 会话固定 | **必开**（943 为 Session 固定攻击，非 SQLi） |
| 944 | Java 攻击 | Java 栈启用 |
| 949/959 | 评分拦截 / 响应阻断评估 | blocking 模式配合阈值 |
| 950–954/956 | 响应信息泄露族（通用/SQL/Java/PHP/IIS/Ruby 泄露） | 数据出口站点启用 |
| 955 | Webshell | 建议启用 |
| 913/915/911 | 爬虫检测 / 请求体限制 / 方法限制 | 按需 |
| 900/905/980/999 | CRS 前排除/通用异常/事件关联/CRS 后通用排除（普通可选组，不选不加载；发射时强制的仅 901/949（959 仅在开启响应检测 waf_check_response 时强制）。900 仅随包 .conf.example,改名启用才有实文件） | 按需 |
...
| 自定义规则 conditions | 每请求正则评估 | 用精确 target（uri/args/headers/user_agent/body）而非宽字段 |
...
| 指标采集间隔 `metrics_interval` | 配置文件（默认 30s，最小 5s） | 常规 30s，排障期临时调小 |
