...
| 409 | 冲突（重名、并发操作） |
| 413 | 请求体超限 |
| 428 | 需 MFA 验证码（MFA 写保护触发的写操作） |
| 429 | 请求过于频繁（登录/注册/集群公开端点限流、账户锁定） |
| 500 | 服务端错误 |

> **口径约定**：上表通用错误码对各端点全局适用；动态端点文档（`/api/v1/docs`）的每行 `errors` 仅登记该端点**业务特有**的错误形态（如 400 port_or_domain_conflict 域名/端口冲突、428 MFA 验证），通用码不逐行重复列出。
