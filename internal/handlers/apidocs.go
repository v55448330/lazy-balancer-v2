...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/cluster/sync/waf-files", "集群机器接口", "按需拉取 WAF 规则文件包", "", `{"code":0,"data":{"crs_version":"v4.28.0","crs_sha256":"...","ip2region_version":"v3.17.0","ip2region_sha256":"...","crs_tar_gz":"...","xdb":"..."}}`, []string{"401 集群凭证无效", "404 主节点无 WAF 规则文件"}, "X-Cluster-Token；快照只携带文件哈希引用，从节点本地哈希不一致时才调用本端点拉取完整 CRS 规则树与 GeoIP 数据库，内容与快照引用哈希核验后落盘。"},
	...
	{"GET", "/security/policies", "安全", "安全策略列表", "", `[{"id":1,"name":"默认策略","mode":"blocking","enabled":true,"rule_count":3,"has_waf":true,"has_ip_control":true,"has_rate_limit":false,"ip_acl_enabled":true}]`, []string{"401 unauthenticated"}, ""},
