...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/security/ip-lists", "安全", "IP 地址列表列表", "", `[{"id":1,"name":"办公网","description":"","category":"内网","entries":[{"value":"10.0.0.0/8","remark":"内网段"}],"entry_count":1,"ref_count":2,"ref_policies":[{"id":1,"name":"默认策略"}]}]`, []string{"401 unauthenticated"}, "ref_count/ref_policies 为引用该列表的安全策略（ip_acl_list_refs ∪ ip_whitelist_refs，JSON 精确解析）。"},
	...
	{"GET", "/security/threat-lib", "安全", "威胁情报库源列表与合并生效数", "", `{"sources":[{"id":1,"name":"ustc","display_name":"中科大黑 IP","url":"https://blackip.ustc.edu.cn/list.php?txt","format":"plain","update_enabled":true,"apply_enabled":true,"entry_count":14030,"version":"2026.09.23","update_status":"success","message":"","last_checked":"2026-09-23 02:00:00","next_update":"2026-09-24 02:00:00"}],"merged_apply_count":19144}`, []string{"401 unauthenticated", "500 数据库故障"}, "内置只读三源（USTC 黑 IP/FireHOL level1/ET Compromised）；merged_apply_count=apply_enabled 源条目合计。版本=最近成功更新日期（YYYY.MM.DD）。"},
	{"PUT", "/security/threat-lib/:id/flags", "安全", "更新威胁库源开关", `{"update_enabled":true,"apply_enabled":false}`, `{"code":0,"message":"已更新"}`, []string{"400 无可更新字段", "401 unauthenticated", "403 slave_or_admin_required", "404 源不存在"}, "仅 update_enabled/apply_enabled 两字段可改，请求体携带的 url/name/format 一律忽略（内置源不可变造）。apply 变更即刻重建合并文件并触发主节点重载。"},
	...
	{"GET", "/security/threat-lib/:id/entries", "安全", "威胁库源条目分页查看", "", `{"total":14030,"page":1,"size":200,"entries":["1.0.1.0/24"]}`, []string{"401 unauthenticated", "404 源不存在或尚无已下载数据"}, "query: page（默认 1）、size（默认 200，上限 1000）。只读：更新产出的固定条目列表。"},
	{"GET", "/security/threat-lib/:id/export", "安全", "导出威胁库源条目", "", "", []string{"401 unauthenticated", "404 源不存在或尚无已下载数据"}, "text/plain 附件下载（threat-<name>-<YYYY.MM.DD>.txt），导出行径写审计日志。"},
}
