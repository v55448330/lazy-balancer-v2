...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/auth/mfa/verify-step", "认证", "MFA step-up 验证", `{"code":"123456"}`, `{"token":"jwt(mfa_ts刷新)","user":{},"node_mode":"master"}`, []string{"400 invalid_request", "401 wrong_code_or_invalid_challenge"}, "全局写操作验证开启时，写操作返回 428 后凭本端点刷新 JWT 重试。仅接受 6 位动态验证码（恢复代码仅限登录与重置 MFA 入口）。"},
