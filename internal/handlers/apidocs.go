...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/auth/mfa/setup", "认证", "生成 MFA 绑定密钥", `{"code":"123456"}`, `{"secret":"base32","uri":"otpauth://totp/..."}`, []string{"401 wrong_code（重新绑定）"}, "生成待激活密钥（pending），需经 activate 验证后才生效。首次绑定无需请求体；已启用 MFA 的用户重新 setup 须提交 6 位动态验证码确认（2026-09 起登录后无密码输入）。"},
...
var apiDocContracts = map[string]apiDocContract{
	...
	"GET /security/events":                 {queryParameters: []apiDocParameter{{"page", "integer", "页码，默认 1"}, {"page_size", "integer", "每页数量，默认 20，1-100"}, {"action", "string", "按动作过滤（如 block/deny）"}, {"ip", "string", "按完整客户端 IP 精确匹配"}, {"rule_caddy_id", "string", "按规则 Caddy ID 过滤"}, {"rule_name", "string", "按负载规则名称过滤（子串，匹配事件时快照）"}, {"policy_name", "string", "按安全策略名称过滤（子串，匹配事件时快照）"}, {"rule_triggered", "string", "按触发规则过滤（类别：IP 访问控制/地域拦截/WAF 规则（CRS）/自定义规则；也可输细标签或 CRS 6 位规则 ID/消息关键词）"}, {"rule_triggered_exclude", "string", "按触发规则反向过滤（同 rule_triggered 的类别/细标签/ID，整体取反）"}, {"uri", "string", "按请求 URI 过滤（子串）"}, {"start_time", "string", "开始时间（配置时区，YYYY-MM-DD[ HH:MM:SS]）"}, {"end_time", "string", "结束时间（配置时区，YYYY-MM-DD[ HH:MM:SS]）"}}},
