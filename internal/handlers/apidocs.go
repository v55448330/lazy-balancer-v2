...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/security/rate-limit-blocks", "安全", "各站点限流拦截累计次数", "", `{"total":12,"hosts":[{"host":"go029.com","count":9}]}`, []string{"401 unauthenticated"}, "数据来自 Caddy /metrics 的 429 计数，仅按 code=429 过滤（含上游自返 429），自最近一次 Caddy 配置重载以来累计（SECLB34-ACL-1 第 34 轮实证：任何 /load 重建 metrics registry，计数随重载归零），进程重启归零；采集失败返回 500。"},
