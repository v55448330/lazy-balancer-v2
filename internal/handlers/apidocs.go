...
var apiDocRoutes = []apiDocRoute{
	{"POST", "/auth/login", "认证", "用户登录", `{"username":"admin","password":"..."}`, `{"token":"jwt","user":{},"node_mode":"master"}`, []string{"400 invalid_request", "401 invalid_credentials", "429 too_many_attempts_or_locked"}, "密码仅用于验证，不返回。启用 MFA 的用户返回 mfa_required 形态，凭 mfa_token 调 /auth/mfa/verify 换 JWT。"},
	...
	{"POST", "/auth/mfa/verify-step", "认证", "MFA step-up 验证", `{"code":"123456"}`, `{"token":"jwt(mfa_ts刷新)","user":{},"node_mode":"master"}`, []string{"400 invalid_request", "401 wrong_code"}, "全局写操作验证开启时，写操作返回 428 后凭本端点刷新 JWT 重试。仅接受 6 位动态验证码（恢复代码仅限登录与重置 MFA 入口）。"},
	{"POST", "/users/:id/mfa/reset", "认证", "重置用户 MFA（管理员）", `{"code":"123456"}`, `{"code":0,"message":"已重置用户 MFA（用户需重新绑定）"}`, []string{"401 wrong_code", "403 admin_required", "404 not_found"}, "仅 admin；清除该用户全部 MFA 状态并审计留痕。操作者自己启用 MFA 时须提交本人有效 6 位动态验证码（body {code}）。"},
