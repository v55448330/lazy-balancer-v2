...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/auth/mfa/disable", "认证", "禁用 MFA（双重确认）", `{"code":"123456"}`, `{"code":0,"message":"MFA 已禁用"}`, []string{"400 invalid_request", "401 wrong_password_or_code"}, "需当前密码 + 有效验证码，防会话劫持后一键关闭。仅接受 6 位动态验证码（恢复代码仅限登录与重置 MFA 入口）。"},
	{"POST", "/auth/mfa/recovery-codes", "认证", "重新生成恢复代码", ``, `{"code":0,"data":{"recovery_codes":["...×10"]}}`, []string{"400 invalid_request", "401 wrong_password"}, "旧恢复码全部作废。"},
