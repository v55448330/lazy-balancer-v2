...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/certificate-configs", "证书", "创建 DNS 提供商配置", `{"name":"dnspod","dns_provider":"dnspod","dns_credentials":{"id":"...","token":"..."},"enabled":true}`, `{"id":1}`, []string{"400 invalid_request"}, "凭证保存后非管理员仅见掩码形态（更新回传掩码按未改动处理），仅管理员可修改。"},
	...
	{"POST", "/cluster/nodes/:id/login-ticket", "集群", "生成登录票据", "", `{"ticket":"...","url":"https://10.0.0.2:8000"}`, []string{"403 仅主节点管理员", "409 节点不在线"}, "admin JWT 或管理员 API Key；票据 90 秒有效且只能使用一次(时钟偏移预留)。客户端必须将返回地址拼接为 URL#login_ticket=<percent-encoded-ticket>；从节点消费后无论成功或失败都必须清除 fragment。"},
	...
	{"POST", "/cluster/forget-pins", "集群", "清除主节点证书指纹钉", "", `{"code":0,"message":"已清除全部主节点证书指纹，下次同步将按当前证书重新钉扎"}`, []string{"500 清除失败"}, "admin JWT 或管理员 API Key；从节点 PinMismatch 补救：清空 TOFU 指纹（内存+磁盘），下次同步重新钉扎；操作留审计。"},
	...
	{"POST", "/config/import/v1", "配置", "导入 v1（nginx 版）备份", "v1 备份 JSON", `{"message":"已导入 26 条规则"}`, []string{"400 invalid_backup", "403 slave_or_admin_required", "500 rollback"}, "仅主节点；自动转换负载规则（含内联证书），仅导入规则部分；覆盖导入会清理旧规则及其安全策略绑定。"},
