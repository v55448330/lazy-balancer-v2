...
var apiDocRoutes = []apiDocRoute{
	...
	{"PUT", "/rules/:caddy_id", "规则", "更新负载均衡规则", `{"ip_acl_mode":"deny","ip_acl_list":["198.51.100.0/24"],"custom_routes_enabled":true,"path_rules":[{"match_type":"exact","path":"/health","sort_order":0,"upstreams":null}]}`, `{"code":0,"message":"Rule updated"}`, []string{"400 validation_failed", "404 not_found", "409 cert_job_active"}, "path_rules 在每次提交时整体替换；仅提交 ACL 字段时保留未提交的新字段。"},
	...
	{"GET", "/cluster/nodes", "集群", "主节点查看从节点", "", `{"code":0,"data":[{"id":2,"name":"slave-a","ip_address":"10.0.0.2","port":8000,"status":"online","is_approved":true,"reported_version":3,"current_version":3,"health":{"caddy_ok":true,"rules_count":5,"certs_expiring_30d":0,"last_sync_at":"2026-07-18T12:00:00Z","last_sync_error":"","uptime_sec":3600},"last_seen":"2026-07-18T12:00:00Z","created_at":"2026-07-18T11:00:00Z"}]}`, []string{"403 仅主节点管理员"}, "admin JWT；离线状态按 last_seen 超过 2×sync_interval 在读取时计算。"},
	...
	{"PUT", "/cluster/settings", "集群", "更新集群设置", `{"sync_interval":60,"sync_caddy_config":true}`, `{"code":0,"message":"集群设置已更新"}`, []string{"400 参数无效", "403 从节点不能修改 Caddy 同步开关"}, "admin JWT；sync_interval 由主节点下发，sync_caddy_config 仅主节点可改，从节点均只读。"},
	{"GET", "/cluster/sync/snapshot", "集群机器接口", "拉取全量集群快照", "", `{"code":0,"data":{"version":3,"fingerprint":"sha256","signature":"hmac-sha256","rules":[],"users":[],"api_keys":[],"basic_settings":{},"certs":[]}}`, []string{"304 版本和指纹均未变化", "400 快照缺少签名", "401 集群凭证无效"}, "X-Cluster-Token 或 Bearer cluster token；query: since_version,fingerprint。signature 为以节点令牌为键的 HMAC-SHA256（含版本号），从节点强制验签并拒绝版本回退。用户密码哈希、API key 哈希和证书私钥只通过此机器接口传输。"},
	{"POST", "/cluster/sync/pull", "集群", "从节点立即同步", "", `{"code":0,"message":"手动同步完成","data":{"applied_version":3,"changed":true}}`, []string{"500 同步失败"}, "admin JWT；有变更记录应用版本，无变更记录「配置无变化」，均写入操作日志。"},
