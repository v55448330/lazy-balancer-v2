...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/auth/mfa/setup", "认证", "生成 MFA 绑定密钥", `{"password":"...","code":"123456"}`, `{"secret":"base32","uri":"otpauth://totp/..."}`, []string{"401 unauthenticated_or_wrong_password_or_code（重新绑定）"}, "生成待激活密钥（pending），需经 activate 验证后才生效。首次绑定无需请求体；已启用 MFA 的用户重新 setup 须提交密码 + 6 位动态验证码双重确认（防会话劫持后轮换第二因子）。"},
	...
	{"POST", "/auth/mfa/disable", "认证", "禁用 MFA（双重确认）", `{"password":"...","code":"123456"}`, `{"code":0,"message":"MFA 已禁用"}`, []string{"400 invalid_request", "401 wrong_password_or_code"}, "需当前密码 + 有效验证码，防会话劫持后一键关闭。仅接受 6 位动态验证码（恢复代码仅限登录与重置 MFA 入口）。"},
