...
var apiDocRoutes = []apiDocRoute{
	...
	{"PUT", "/cluster/settings", "集群", "更新集群设置", `{"sync_interval":60,"sync_global_config":true,"sync_users":true,"sync_rules":true,"sync_waf_files":true,"sync_security":true}`, `{"code":0,"message":"集群设置已更新"}`, []string{"400 参数无效/恒同步项禁用", "403 从节点不能修改 Caddy 同步开关"}, "admin JWT 或管理员 API Key；五类同步开关默认全开：全局配置/系统数据/负载规则/CRS与IP2Region数据库/安全策略及自定义规则；系统数据（用户/密钥/ACME）恒同步不可禁用——置 false 返回 400。从节点逐节比对哈希，一致跳过并记录操作日志。仅主节点可改。"},
