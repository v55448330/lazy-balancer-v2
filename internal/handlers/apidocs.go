...
var apiDocRoutes = []apiDocRoute{
	...
	{"PUT", "/rules/:caddy_id", "规则", "更新负载规则", `{"custom_routes_enabled":true,"path_rules":[{"match_type":"exact","path":"/health","upstream_path":"","sort_order":0,"upstreams":null}]}`, `{"code":0,"message":"Rule updated"}`, []string{"400 validation_failed", "404 not_found", "409 cert_job_active", "500 caddy_apply_failed"}, "合并更新：指针字段省略时保留原值，显式 false、0 或空数组会写入；host_header/description/dns_server/health_check_path 省略=保留原值、空串=清空；path_rules 仅在字段存在时整体替换。upstreams 为全量替换，须携带完整字段（含 enabled，缺省会被置为 false 禁用该上游）。Caddy 应用/CLI 校验失败返回 400（数据库未写入）。上游对象的 dynamic_dns 字段已废弃（不参与渲染），动态上游以规则级 dynamic_dns 为准。"},
	...
	{"GET", "/security/rules/:caddy_id/stage-stats", "安全", "单规则三阶段安全统计（流程抽屉计数）", "", `{"stage1_blocked_24h":3,"stage3_blocked_24h":5,"ratelimit_blocks_reload":9}`, []string{"401 unauthenticated", "404 not_found", "500 metrics_unavailable"}, "阶段 1/3=近 24h blocked 事件按触发 id 形状分桶（阶段 1={2,4,7,8}∪[800000,900000)，其余归阶段 3）；ratelimit_blocks_reload 为 429 计数按规则域名映射求和，自最近一次 Caddy 配置重载以来累计。caddy_id 不存在返回 404。"},
	{"POST", "/security/policies/batch-bind", "安全", "批量绑定安全策略（规则多选）", `{"rule_ids":["lb_a","lb_b"],"policy_ids":[1,2],"mode":"merge"}`, `{"bound":2,"skipped":[{"rule_id":"lb_t","reason":"TCP 规则不经过安全链"}]}`, []string{"400 validation_failed", "403 slave_or_admin_required"}, "单事务一次渲染。mode=merge（默认，现有∪请求去重后 ≤5，超限的规则进 skipped）或 replace（全量替换为请求集，≤5）；策略存在性一次校验不全在则 400；TCP/不存在规则进 skipped。"},
	{"POST", "/security/test-events", "安全", "生成安全测试事件（管理员测试辅助，UI-only）", "", `{"inserted":12,"hint":"测试数据计入安全总览与阶段统计，可用 DELETE /security/test-events 清除"}`, []string{"401 unauthenticated", "403 slave_or_admin_required"}, "写入 12 条标记为规则 lb_testevent 的模拟安全事件：覆盖阶段 1（IP ACL/黑名单/白名单/遗留 GeoIP/预检 800xxx）、阶段 3（CRS/评分/自定义/请求体/检测 logged）、多样 source_ip（含 IPv6）与时间分布（近 1h/-20h/-30h）；rule_msg 含逗号/中文/空串形状。事件计入安全总览与阶段统计，可用 DELETE 端点清除。"},
	{"DELETE", "/security/test-events", "安全", "清除安全测试事件（管理员测试辅助，UI-only）", "", `{"deleted":24}`, []string{"401 unauthenticated", "403 slave_or_admin_required"}, "仅删除标记为规则 lb_testevent 的测试事件行，返回删除数；真实事件不受影响。"},
