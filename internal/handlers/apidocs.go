...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/security/rate-limit-blocks", "安全", "各站点限流拦截累计次数", "", `{"total":12,"hosts":[{"host":"go029.com","count":9}]}`, []string{"401 unauthenticated"}, "数据来自 Caddy /metrics 的 429 计数，仅按 code=429 过滤（含上游自返 429），自 Caddy 进程启动以来累计，重启归零；采集失败返回 500。"},
