...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/rules", "规则", "负载均衡规则列表", "", `[{"caddy_id":"lb_...","name":"example","protocol":"http","listen_port":80,"custom_routes_enabled":true,"proxy_dial_timeout":0,"proxy_response_header_timeout":0,"proxy_read_timeout":0,"proxy_write_timeout":0,"proxy_stream_timeout":0,"proxy_flush_interval":0,"proxy_stream_close_delay":0,"path_rules":[{"id":1,"match_type":"prefix","path":"/prometheus/","sort_order":0,"upstreams":null}],"enabled":true}]`, []string{"401 unauthenticated"}, "path_rules.upstreams 为 null 时继承父规则上游，否则使用 address、port、weight、protocol 对象数组；TCP 规则可用 tcp_proxy_protocol=true 向上游发送 PROXY v2 协议头传递真实客户端 IP。"},
	...
	{"GET", "/cluster/sync/waf-files", "集群机器接口", "按需拉取 WAF 规则文件包", "", `{"code":0,"data":{"crs_version":"v4.14.0","crs_sha256":"...","ip2region_version":"v3.17.0","ip2region_sha256":"...","crs_tar_gz":"...","xdb":"..."}}`, []string{"401 集群凭证无效", "404 主节点无 WAF 规则文件"}, "X-Cluster-Token；快照只携带文件哈希引用，从节点本地哈希不一致时才调用本端点拉取完整 CRS 规则树与 GeoIP 数据库，内容与快照引用哈希核验后落盘。"},
