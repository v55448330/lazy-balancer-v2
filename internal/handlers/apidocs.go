...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/security/events/count", "安全", "IP 事件计数（弹框上下文）", "", `{"count":2}`, []string{"400 缺少 ip"}, "query: ip（完整客户端 IP 精确匹配）, days（默认 30，1-365）。走 client_ip+event_time 索引范围 COUNT，近 N 天该 IP 的安全事件总数（含 blocked 与 logged）。"},
