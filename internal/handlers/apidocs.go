...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/rules", "规则", "负载均衡规则列表", "", `[{"caddy_id":"lb_...","name":"example","protocol":"http","listen_port":80,"ip_acl_mode":"allow","ip_acl_list":["192.0.2.0/24"],"custom_routes_enabled":true,"proxy_dial_timeout":0,"proxy_response_header_timeout":0,"proxy_read_timeout":0,"proxy_write_timeout":0,"proxy_stream_timeout":0,"proxy_flush_interval":0,"proxy_stream_close_delay":0,"path_rules":[{"id":1,"match_type":"prefix","path":"/prometheus/","sort_order":0,"upstreams":null}],"enabled":true}]`, []string{"401 unauthenticated"}, "ip_acl_mode/ip_acl_list 已弃用（由安全策略取代），始终返回默认值。path_rules.upstreams 为 null 时继承父规则上游，否则使用 address、port、weight、protocol 对象数组；TCP 规则可用 tcp_proxy_protocol=true 向上游发送 PROXY v2 协议头传递真实客户端 IP。"},
	{"POST", "/rules", "规则", "创建负载均衡规则", `{"name":"example","protocol":"http","listen_port":8080,"custom_routes_enabled":true,"proxy_dial_timeout":5,"proxy_response_header_timeout":15,"proxy_read_timeout":30,"proxy_write_timeout":30,"proxy_stream_timeout":0,"proxy_flush_interval":-1,"proxy_stream_close_delay":0,"path_rules":[{"match_type":"prefix","path":"/prometheus/","sort_order":0,"upstreams":[{"address":"127.0.0.1","port":9090,"weight":1,"protocol":"http"}]}],"upstreams":[{"host":"127.0.0.1","port":9000}]}`, `{"caddy_id":"lb_..."}`, []string{"400 validation_failed", "403 slave_mode"}, "路径必须以 / 开头，match_type 仅支持 prefix 或 exact；代理超时单位为秒，0 表示继承全局配置；proxy_flush_interval：0=自动（仅 text/event-stream 触发立即刷新），-1=立即刷新所有响应（无缓冲），>0=每 N 秒刷新一次；proxy_stream_close_delay：0=reload 时立即关闭旧流，>0=延迟 N 秒关闭。"},
	{"GET", "/rules/:caddy_id", "规则", "规则详情", "", `{"caddy_id":"lb_...","name":"example","ip_acl_mode":"","ip_acl_list":[],"custom_routes_enabled":false,"proxy_dial_timeout":0,"proxy_response_header_timeout":0,"proxy_read_timeout":0,"proxy_write_timeout":0,"proxy_stream_timeout":0,"proxy_flush_interval":0,"proxy_stream_close_delay":0,"path_rules":[]}`, []string{"404 not_found"}, ""},
	...
	{"PUT", "/rules/:caddy_id", "规则", "更新负载均衡规则", `{"custom_routes_enabled":true,"path_rules":[{"match_type":"exact","path":"/health","sort_order":0,"upstreams":null}]}`, `{"code":0,"message":"Rule updated"}`, []string{"400 validation_failed", "404 not_found", "409 cert_job_active", "500 caddy_apply_failed"}, "合并更新：指针字段省略时保留原值，显式 false、0 或空数组会写入；path_rules 仅在字段存在时整体替换；字符串空值保留原值。Caddy 应用失败返回 500。"},
	...
	{"GET", "/security/custom-rules", "安全", "自定义规则列表", "", `[{"id":1,"name":"管理路径保护","conditions":[{"target":"uri","operator":"starts_with","pattern":"/admin"}],"action":"block","score":5,"status_code":403,"enabled":true}]`, []string{"401 unauthenticated"}, ""},
	{"POST", "/security/custom-rules", "安全", "创建自定义规则", `{"name":"管理路径保护","conditions":[{"target":"uri","operator":"starts_with","pattern":"/admin"}],"action":"block","score":5,"status_code":403,"enabled":true}`, `{"id":1}`, []string{"400 validation_failed", "403 slave_or_admin_required"}, "conditions 数组支持多条件（AND 关系）。"},
	{"PUT", "/security/custom-rules/:id", "安全", "更新自定义规则", `{"name":"管理路径保护","conditions":[{"target":"uri","operator":"starts_with","pattern":"/admin"}],"action":"block","score":5,"status_code":403,"enabled":true}`, `{"code":0}`, []string{"400 validation_failed", "403 slave_or_admin_required", "404 not_found"}, ""},
	...
	{"GET", "/security/ip2region", "安全", "IP2Region 数据库信息", "", `{"version":"v3.17.0","auto_update":true,"update_status":"success"}`, []string{"401 unauthenticated"}, ""},
