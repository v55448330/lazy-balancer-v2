...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/config/validate", "配置", "验证 Caddy 配置", `{}`, `{"code":0,"message":"配置有效"}`, []string{"400 config_invalid", "403 admin_required"}, ""},
	...
	{"DELETE", "/rules/:caddy_id", "规则", "删除负载规则", "", `{"code":0,"message":"Rule deleted"}`, []string{"404 not_found", "409 cert_job_running", "400 caddy_validation_failed"}, "有 worker 正在执行签发时秒回 409（queued/冷却中不拦，稍后重试即可）；无在途任务的删除即时完成。"},
	...
	{"POST", "/rules/:caddy_id/duplicate", "规则", "复制规则", "", `{"caddy_id":"lb_new..."}`, []string{"404 not_found", "400 caddy_validation_failed"}, "不读取请求体；名称固定追加「（副本）」后缀。"},
	...
	{"PUT", "/security/crs/auto-update", "安全", "开关 CRS 自动更新", `{"auto_update":true}`, `{"code":0}`, []string{"403 slave_or_admin_required", "400 caddy_validation_failed"}, ""},
	{"POST", "/security/crs/update", "安全", "手动触发 CRS 更新", "", `{"status":"running","trigger":"manual"}`, []string{"403 slave_or_admin_required", "409 update_running", "400 caddy_validation_failed"}, ""},
	...
	{"PUT", "/security/ip2region/auto-update", "安全", "开关 IP2Region 自动更新", `{"auto_update":true}`, `{"code":0}`, []string{"403 slave_or_admin_required", "400 caddy_validation_failed"}, ""},
	{"POST", "/security/ip2region/update", "安全", "手动触发 IP2Region 更新", "", `{"status":"running","trigger":"manual"}`, []string{"403 slave_or_admin_required", "409 update_running", "400 caddy_validation_failed"}, ""},
