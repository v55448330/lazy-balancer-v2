...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/config/export", "配置", "导出配置备份（lbbak）", "", `{"meta":{},"config":{},"tables":{}}`, []string{"403 slave_or_admin_or_api_key_read_only", "428 mfa_step_up_required", "413 body_exceeds_48MB"}, "仅主节点；只读 API Key 不能导出配置备份（403）；开启 mfa_write_guard 时 JWT 身份需新鲜 MFA 验证（428，经 verify-step 后重试）。下载 lbbak 备份包（tar.gz 二进制，含用户与密钥哈希、证书任务；勾选安全防护分类时附 CRS/IP2Region 规则库文件本体，示例为包内 config.json 形态）。"},
	...
	{"GET", "/auto-backup/:id/download", "配置", "下载备份文件", "", "(lbbak 二进制流)", []string{"400 invalid_id", "403 slave_or_admin_required", "404 not_found"}, "与手动导出同敏感级（备份含私钥与凭证明文），下载审计留痕。"},
