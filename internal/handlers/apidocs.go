...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/api-keys", "API密钥", "API 密钥列表", "", `[{"id":1,"name":"ci","username":"admin","is_enabled":true}]`, []string{"401 unauthenticated"}, "所有已登录用户可读。"},
	...
	{"GET", "/cluster/nodes", "集群", "主节点查看从节点", "", `{"code":0,"data":[{"id":2,"name":"slave-a","ip_address":"10.0.0.2","port":8000,"status":"online","is_approved":true,"reported_version":3,"current_version":3,"health":{"caddy_ok":true,"rules_count":5,"certs_expiring_30d":0,"last_sync_at":"2026-07-18T12:00:00Z","last_sync_error":"","uptime_sec":3600},"last_seen":"2026-07-18T12:00:00Z","created_at":"2026-07-18T11:00:00Z"}]}`, []string{"401 unauthenticated"}, "所有已登录用户可读；仅接受 JWT，仅主节点返回节点列表；离线状态按 last_seen 超过 2×sync_interval 在读取时计算。"},
