...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/cluster/service-control", "集群机器接口", "从节点执行服务控制", `{"action":"stop_caddy","ticket":"base64url(payload).base64url(signature)"}`, `{"code":0,"message":"Caddy 已停止"}`, []string{"400 invalid_request", "401 服务控制票据无效或已过期"}, "由主节点调用；凭 90 秒一次性 HMAC-SHA256 票据授权（密钥为集群令牌哈希，与动作和目标节点绑定，防重放），不使用集群令牌头。"},
