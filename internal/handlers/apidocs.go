...
var apiDocRoutes = []apiDocRoute{
	...
	{"POST", "/auth/mfa/verify", "认证", "MFA 登录第二步验证", `{"mfa_token":"<登录返回的挑战令牌>","code":"123456 或恢复代码"}`, `{"token":"jwt","user":{},"node_mode":"master"}`, []string{"400 invalid_request", "401 invalid_challenge_or_code", "429 too_many_attempts"}, "公开接口（同 loginRateLimit）；启用 MFA 的用户密码验证通过后先收到 mfa_required+mfa_token，再凭本端点换取 JWT。挑战令牌 5 分钟单次。"},
