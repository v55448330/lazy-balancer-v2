...
var apiDocRoutes = []apiDocRoute{
	...
	{"DELETE", "/rules/:caddy_id", "规则", "删除负载规则", "", `{"code":0,"message":"Rule deleted"}`, []string{"404 not_found", "409 cert_job_running"}, "有 worker 正在执行签发时秒回 409（queued/冷却中不拦，稍后重试即可）；无在途任务的删除即时完成。"},
	...
	{"POST", "/rules/:caddy_id/disable", "规则", "禁用规则", "", `{"code":0,"message":"Rule disabled"}`, []string{"404 not_found", "409 cert_job_running", "500 caddy_apply_failed"}, "非终态证书任务会置为 disabled。有 worker 正在执行签发时秒回 409（queued/冷却中不拦，稍后重试即可）。"},
	...
	{"DELETE", "/security/ip-lists/:id", "安全", "删除 IP 地址列表", "", `{"code":0}`, []string{"403 slave_or_admin_required", "404 not_found", "409 referenced"}, "被安全策略引用时返回 409，需先解除引用。"},
