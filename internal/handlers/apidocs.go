...
var apiDocRoutes = []apiDocRoute{
	...
	{"GET", "/security/rate-limit-blocks", "安全", "各站点限流拦截累计次数", "", `{"total":12,"hosts":[{"host":"go029.com","count":9}]}`, []string{"401 unauthenticated"}, "数据来自 Caddy /metrics 的 429 计数，仅按 code=429 过滤（含上游自返 429），自最近一次 Caddy 配置重载以来累计，进程重启归零；采集失败返回 500。"},
	{"GET", "/security/rules/:caddy_id/stage-stats", "安全", "单规则三阶段安全统计（流程抽屉计数）", "", `{"stage1_blocked_24h":3,"stage3_blocked_24h":5,"ratelimit_blocks_reload":9}`, []string{"401 unauthenticated", "500 metrics_unavailable"}, "阶段 1/3=近 24h blocked 事件按触发 id 形状分桶（阶段 1={2,4,7,8}∪[800000,900000)，其余归阶段 3）；ratelimit_blocks_reload 为 429 计数按规则域名映射求和，自最近一次 Caddy 配置重载以来累计。"},
	...
	{"POST", "/rules/batch-block-pages", "规则", "批量设置阶段拦截页", `{"rule_ids":["lb_a","lb_b"],"block_page_stage1_id":7,"block_page_stage1_status":403,"block_page_stage3_id":0,"block_page_stage3_status":0}`, `{"bound":2,"skipped":[{"rule_id":"lb_x","reason":"规则不存在"}]}`, []string{"400 validation_failed", "403 slave_or_admin_required"}, "单事务一次渲染，逐规则 UPDATE 4 列（0=跟随策略）。页 id>0 须存在于 security_block_pages；状态码 ∈ {0,400,401,403,404,503}。"},
	{"POST", "/security/policies/:id/split", "安全", "混合策略一键拆分迁移", "", `{"created":[{"id":9,"name":"默认策略（阶段 1）","policy_type":"stage1"}],"remapped":2,"skipped":[],"deleted_original":true}`, []string{"400 not_mixed", "404 not_found"}, "仅 mixed 策略：按特征组生成「原名（阶段 N）」单职子策略（空组不生成，阶段 1/3 继承拦截页，阶段 2 恒 429 不配页）；全量重映射绑定（合并后 >5 条的规则进 skipped 保留原绑定）；无 skipped 才删除原策略。单事务一次渲染。"},
	...
	{"GET", "/security/events/count", "安全", "IP 事件计数（弹框上下文）", "", `{"count":2}`, []string{"400 缺少 ip", "500 查询失败"}, "query: ip（完整客户端 IP 精确匹配）, days（默认 30，1-365）。走 client_ip+event_time 索引范围 COUNT，近 N 天该 IP 的安全事件总数（含 blocked 与 logged）。"},
