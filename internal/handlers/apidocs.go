...
var apiDocRoutes = []apiDocRoute{
	...
	{"PUT", "/rules/:caddy_id", "规则", "更新负载规则", `{"custom_routes_enabled":true,"path_rules":[{"match_type":"exact","path":"/health","sort_order":0,"upstreams":null}]}`, `{"code":0,"message":"Rule updated"}`, []string{"400 validation_failed", "404 not_found", "409 cert_job_active", "500 caddy_apply_failed"}, "合并更新：指针字段省略时保留原值，显式 false、0 或空数组会写入；host_header/description/dns_server/health_check_path 省略=保留原值、空串=清空；path_rules 仅在字段存在时整体替换。upstreams 为全量替换，须携带完整字段（含 enabled，缺省会被置为 false 禁用该上游）。Caddy 应用/CLI 校验失败返回 400（数据库未写入）。上游对象的 dynamic_dns 字段已废弃（不参与渲染），动态上游以规则级 dynamic_dns 为准。"},
