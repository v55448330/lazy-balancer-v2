...
var toolUsage = map[string]string{
	...
	"export_config":                "导出完整配置备份（lbbak tar.gz 二进制包，MCP 通道不支持二进制下载——调用返回错误指引，请经管理面板或 REST /config/export 下载）。需管理员权限且仅主节点可用；只读 API Key 被拒绝（403）。",
	...
	"get_crs_rule_index":           "查看 CRS 全部规则的结构化索引（6 位规则 ID/消息/文件/分类），配置排除规则与规则组校验的数据源",
	...
	"bind_security_policy":         "将策略绑定到规则（rule_caddy_id）；规则可绑定多条安全策略（按 policy_id ASC 顺序评估，最多 8 条；超限时请使用 set_rule_security_policies 整体替换）",
	"unbind_security_policy":       "解除策略与规则的绑定（仅解除该策略，同规则的其他策略绑定保留）",
	"set_rule_security_policies":   "原子设置规则的安全策略集合（最多 8 条，按 policy_id ASC 顺序评估，整体替换现有绑定；policy_ids 为空数组 [] = 解除该规则全部绑定）",
	...
	"add_ip_to_list":               "向 IP 地址列表追加单条 IP/CIDR（安全事件处置用）：幂等——value 已存在时返回 added=false，新条目备注固定为「事件处置」",
	...
	"control_cluster_node_service": "主节点遥控从节点服务（action：start_caddy/stop_caddy/restart_caddy/restart_app），仅主节点管理员 Key 可用；遇证书指纹不匹配先 forget_cluster_node_pin",
	...
	"delete_user":                  "删除用户",
	"reset_user_mfa":               "重置用户 MFA（仅管理员）：操作者已启用 MFA 时须提交本人 6 位动态验证码（未启用传空串）；MCP/API Key 路径不走 428 step-up，直接在本接口验码",
	...
	"import_config":                "导入 v2 配置备份（覆盖式，先验证后写入）。备份超过 1 MiB 时 MCP 通道不可用（网关体积上限），请改用管理面板或 REST 导入",
	"validate_import":              "验证导入文件是否有效（不写入）。备份超过 1 MiB 时 MCP 通道不可用（网关体积上限），请改用管理面板或 REST 导入",
	"import_v1_config":             "导入 v1（nginx 版）配置，自动转换为 v2 规则。备份超过 1 MiB 时 MCP 通道不可用（网关体积上限），请改用管理面板或 REST 导入",
	"get_rule_caddy_config":        "查看指定规则生成的 Caddy JSON 配置片段",
	"get_rule_metrics_history":     "查看指定规则的历史请求数/状态码/流量趋势（range 参数选窗：1h/6h/24h/7d，默认 24h；跨规则全局趋势用 get_metrics_history）",
	"get_rule_logs":                "查看指定规则最近的访问日志（返回最近 1000 行尾读，无 offset 续读——增量续读请走 REST log-stream 端点）",
