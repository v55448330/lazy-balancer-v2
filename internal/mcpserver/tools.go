...
var toolUsage = map[string]string{
	...
	"create_security_policy":       "创建安全策略：配置 WAF 模式/异常阈值/CRS 规则组/IP ACL/GeoIP 区域控制/限流/拦截页面",
	"update_security_policy":       "更新策略任意字段（如调整 WAF 模式、增减区域、修改阈值）",
	"delete_security_policy":       "删除策略（绑定关系自动解除）",
	"bind_security_policy":         "将策略绑定到规则（rule_caddy_id），一个规则只绑一个策略",
	"unbind_security_policy":       "解除策略与规则的绑定",
	...
	"set_cluster_mode":             "切换集群角色（standalone→master 或 slave→standalone）",
	...
	"get_caddy_logs":               "查看 Caddy 运行日志（access/error，按类型过滤）",
	...
	"get_metrics_history":          "查看全局历史指标趋势（按小时/天聚合）",
