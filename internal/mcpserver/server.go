...
var tools = []toolSpec{
	...
	{"list_security_events", "分页列出安全事件（WAF 拦截/IP ACL 拒绝），可按负载规则/策略/触发规则/URI 过滤", http.MethodGet, "/security/events", nil, []string{"page", "page_size", "action", "ip", "rule_caddy_id", "rule_name", "policy_name", "rule_triggered", "rule_triggered_exclude", "uri", "start_time", "end_time"}, securityEventsSchema},
	...
	// McpB-2(R39):DELETE 请求体恒被后端丢弃——bodySchema(任意字段放行)会误导
	// Agent 构造无效 body;换成路径参数契约形态(required id+caddy_id,
	...
	{"get_rule_logs", "获取指定规则的访问日志（最近 1000 行尾读，无 offset 续读）", http.MethodGet, "/rules/{caddy_id}/logs", []string{"caddy_id"}, nil, idSchema("caddy_id", "规则 Caddy ID", "string")},
	...
	{"get_rule_metrics_history", "获取指定规则的历史指标（range 选窗 1h/6h/24h/7d；跨规则全局趋势用 get_metrics_history）", http.MethodGet, "/rules/{caddy_id}/metrics-history", []string{"caddy_id"}, []string{"range"}, ruleMetricsHistorySchema},
	...
	{"get_metrics_history", "获取历史指标趋势（interval 选窗，可按 rule_id 过滤；单规则固定窗口用 get_rule_metrics_history 的 range）", http.MethodGet, "/metrics/history", nil, []string{"rule_id", "interval"}, metricsHistorySchema},
...
权限范围：
- 只读 Key（read_only）可调用 GET 查询类工具（export_config 除外——导出含用户/密钥哈希，与写操作同级隐藏）及 5 个读探测 POST 工具（test_ca_provider/test_certificate_config/parse_certificate/validate_import/preview_config）；其余写工具（POST/PUT/DELETE）需非只读 Key
- 写操作需要管理员权限（API Key 所属用户角色为 admin），非管理员 Key 调用写工具返回 403
- 从节点上仅集群端点（promote_cluster/pull_sync/set_cluster_mode/forget_cluster_pins）可用且仅从节点语义正确，其余变更类写工具一律 403（预览/解析/测试类 POST 除外）——集群运维在目标节点本身调用，不要发往主节点；所有写操作校验后即时生效（失败自动回滚），无需手动 reload
...
const createRuleSchema = `{"type":"object","required":["name","protocol","listen_port","upstreams"],"properties":{"name":{"type":"string"},"description":{"type":"string"},"protocol":{"type":"string","enum":["http","tcp"]},"domain":{"type":"string"},"listen_port":{"type":"integer","minimum":1,"maximum":65535},"strategy":{"type":"string","enum":["weighted_round_robin","ip_hash","least_conn","random","first","cookie"],"description":"负载策略。HTTP 六值全集；TCP 规则不支持 cookie（后端将拒绝，保存前请改用其余五值）"},"dynamic_dns":{"type":"boolean"},"enable_dns_server":{"type":"boolean"},"dns_server":{"type":"string"},"dns_family":{"type":"string","enum":["ipv4","ipv6","both"]},"health_check_path":{"type":"string"},"health_check_interval":{"type":"integer"},"health_check_timeout":{"type":"integer"},"health_check_unhealthy_threshold":{"type":"integer"},"health_check_healthy_threshold":{"type":"integer"},"enable_active_health_check":{"type":"boolean"},"tcp_health_check_port":{"type":"integer"},"tcp_proxy_protocol":{"type":"boolean"},"tcp_try_duration":{"type":"integer"},"tcp_try_interval":{"type":"integer"},"request_body_max_size_mb":{"type":"integer"},"upstream_keepalive_timeout":{"type":"integer"},"server_tokens_hidden":{"type":"integer"},"custom_routes_enabled":{"type":"boolean"},"proxy_dial_timeout":{"type":"integer","minimum":0},"proxy_response_header_timeout":{"type":"integer","minimum":0},"proxy_read_timeout":{"type":"integer","minimum":0},"proxy_write_timeout":{"type":"integer","minimum":0},"proxy_stream_timeout":{"type":"integer","minimum":0},"proxy_flush_interval":{"type":"integer","minimum":-1},"proxy_stream_close_delay":{"type":"integer","minimum":0},"path_rules":{"type":"array","items":{"type":"object"}},"host_header":{"type":"string"},"upstreams":{"type":"array","items":{"type":"object","required":["host","port"],"properties":{"host":{"type":"string"},"port":{"type":"integer"},"weight":{"type":"integer"},"dynamic_dns":{"type":"boolean"},"enabled":{"type":"boolean"},"protocol":{"type":"string"},"max_connections":{"type":"integer"}}}},"enable_tls":{"type":"boolean"},"tls_source":{"type":"string","enum":["manual","acme_dns"],"description":"证书来源：manual=手动上传证书/私钥（tls_cert/tls_key），acme_dns=ACME DNS 验证签发（需 acme_config_id）"},"acme_config_id":{"type":"integer"},"ca_provider_id":{"type":"integer"},"tls_cert":{"type":"string"},"tls_key":{"type":"string"},"tls_http_redirect":{"type":"boolean"},"enable_compress":{"type":"boolean"},"compress_types":{"type":"string"},"log_enabled":{"type":"boolean"}},"additionalProperties":false}`

const updateRuleSchema = `{"type":"object","required":["caddy_id"],"properties":{"caddy_id":{"type":"string"},"name":{"type":"string"},"description":{"type":"string"},"protocol":{"type":"string"},"domain":{"type":"string"},"listen_port":{"type":"integer"},"strategy":{"type":"string","enum":["weighted_round_robin","ip_hash","least_conn","random","first","cookie"],"description":"负载策略。HTTP 六值全集；TCP 规则不支持 cookie（后端将拒绝，保存前请改用其余五值）"},"dynamic_dns":{"type":"boolean"},"enable_dns_server":{"type":"boolean"},"dns_server":{"type":"string"},"dns_family":{"type":"string"},"health_check_path":{"type":"string"},"health_check_interval":{"type":"integer"},"health_check_timeout":{"type":"integer"},"health_check_unhealthy_threshold":{"type":"integer"},"health_check_healthy_threshold":{"type":"integer"},"enable_active_health_check":{"type":"boolean"},"tcp_health_check_port":{"type":"integer"},"tcp_proxy_protocol":{"type":"boolean"},"tcp_try_duration":{"type":"integer"},"tcp_try_interval":{"type":"integer"},"request_body_max_size_mb":{"type":"integer"},"upstream_keepalive_timeout":{"type":"integer"},"server_tokens_hidden":{"type":"integer"},"custom_routes_enabled":{"type":"boolean"},"proxy_dial_timeout":{"type":"integer","minimum":0},"proxy_response_header_timeout":{"type":"integer","minimum":0},"proxy_read_timeout":{"type":"integer","minimum":0},"proxy_write_timeout":{"type":"integer","minimum":0},"proxy_stream_timeout":{"type":"integer","minimum":0},"proxy_flush_interval":{"type":"integer","minimum":-1},"proxy_stream_close_delay":{"type":"integer","minimum":0},"path_rules":{"type":"array","items":{"type":"object"}},"host_header":{"type":"string"},"upstreams":{"type":"array","items":{"type":"object"}},"enable_tls":{"type":"boolean"},"tls_source":{"type":"string","enum":["manual","acme_dns"],"description":"证书来源：manual=手动证书（tls_cert/tls_key），acme_dns=ACME DNS 验证签发（需 acme_config_id）"},"acme_config_id":{"type":"integer"},"ca_provider_id":{"type":"integer"},"tls_cert":{"type":"string"},"tls_key":{"type":"string"},"tls_http_redirect":{"type":"boolean"},"enable_compress":{"type":"boolean"},"compress_types":{"type":"string"},"enabled":{"type":"boolean"},"log_enabled":{"type":"boolean"}},"additionalProperties":false}`
...
const securityEventsSchema = `{"type":"object","properties":{"page":{"type":"integer","minimum":1,"default":1},"page_size":{"type":"integer","minimum":1,"maximum":100,"default":20},"action":{"type":"string"},"ip":{"type":"string"},"rule_caddy_id":{"type":"string"},"rule_name":{"type":"string","description":"按负载规则名称过滤（子串）"},"policy_name":{"type":"string","description":"按安全策略名称过滤（子串）"},"rule_triggered":{"type":"string","description":"按触发规则过滤（类别：IP 访问控制/地域拦截/WAF 规则（CRS）含全部 6 位规则 ID/自定义规则；也可输细标签（评分拦截/协议异常/协议攻击）或 CRS 规则 ID/消息关键词）"},"rule_triggered_exclude":{"type":"string","description":"按触发规则反向过滤（同 rule_triggered 的类别/细标签/ID，整体取反——筛掉该类型看其余）"},"uri":{"type":"string","description":"按请求 URI 过滤（子串）"},"start_time":{"type":"string","description":"开始时间（配置时区，YYYY-MM-DD[ HH:MM:SS]）"},"end_time":{"type":"string","description":"结束时间（配置时区，YYYY-MM-DD[ HH:MM:SS]）"}},"additionalProperties":false}`
...
func forward(ctx context.Context, client *http.Client, baseURL, internalAuthSecret string, spec toolSpec, call mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// R39-10(C5):v2.3.0 导出恒为 lbbak 二进制 tar.gz(application/gzip)。
	// gzip 字节经 NewToolResultText 会以文本形态静默损坏(UTF-8 替换/截断),
	// 4 MiB 内也必须拒收——给可行动指引而非损坏数据。
	if spec.name == "export_config" && !strings.HasPrefix(strings.TrimSpace(strings.ToLower(response.Header.Get("Content-Type"))), "application/json") {
		result := mcp.NewToolResultText("配置备份为 lbbak 二进制压缩包（tar.gz），MCP 通道不支持二进制下载，请经管理面板或 REST /config/export 下载")
		result.IsError = true
		return result, nil
	}
