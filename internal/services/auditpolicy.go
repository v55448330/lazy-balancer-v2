...
var auditRoutePolicies = map[string]AuditPolicy{
	...
	"POST /api/v1/rules/batch-block-pages":                AuditPolicyExplicit,
	...
	"POST /api/v1/security/policies/batch-bind":           AuditPolicyExplicit,
	"POST /api/v1/security/policies/:id/split":            AuditPolicyExplicit,
	...
	// 安全测试事件（R45 验证辅助）：handler 显式留痕生成/清除（响应含清除提示，
	// 详情写明统计污染面）。
	"POST /api/v1/security/test-events":   AuditPolicyExplicit,
	"DELETE /api/v1/security/test-events": AuditPolicyExplicit,
}
...
func HasExplicitAuditEvent(method, path string) bool {
	...
	case "POST /api/v1/auth/login",
		...
		"POST /api/v1/rules/batch-block-pages",
		...
		"POST /api/v1/security/policies/batch-bind",
		"POST /api/v1/security/policies/:id/split",
		...
		"POST /api/v1/security/test-events",
		"DELETE /api/v1/security/test-events",
