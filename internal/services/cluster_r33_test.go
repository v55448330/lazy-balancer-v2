...
func TestWafBundleSyncDetail_reportsPerComponentOutcome(t *testing.T) {
	...
	tests := []struct {
		...
		{"both updated", true, true, "从主节点同步：CRS 规则已更新至 v4.28.0；IP2Region数据库已更新至 v3.17.0"},
		{"only crs updated", true, false, "从主节点同步：CRS 规则已更新至 v4.28.0；IP2Region数据库已是最新"},
		{"only xdb updated", false, true, "从主节点同步：CRS 规则已是最新；IP2Region数据库已更新至 v3.17.0"},
		{"version missing falls back to bare update", true, false, "从主节点同步：CRS 规则已更新；IP2Region数据库已是最新"},
	}
