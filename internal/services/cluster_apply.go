...
func insertSnapshotUsersAndKeys(ctx context.Context, tx *sql.Tx, snapshot models.ClusterSnapshot) error {
	for _, user := range snapshot.Users {
		if _, err := tx.ExecContext(ctx, `INSERT INTO users (id,username,password_hash,role,display_name,is_enabled,password_version,password_changed_at,created_at,last_login,mfa_enabled,mfa_secret,mfa_recovery_codes,mfa_last_timestep,auth_provider,oidc_subject,oidc_issuer) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
// snapshotTrustDetection 快照行的 trust_detection：缺列（旧主节点快照/旧备份）
// 归 0（直通语义不影响非 stage0 策略——该列仅 stage0 消费）。
func snapshotTrustDetection(p map[string]interface{}) interface{} {
	switch v := p["trust_detection"].(type) {
	case bool:
		return v
	case float64:
		return v != 0
...
func applySecurityTables(ctx context.Context, tx *sql.Tx, snapshot models.ClusterSnapshot) error {
	...
	for _, p := range policies {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO security_policies (id,name,description,mode,anomaly_threshold,ip_acl_mode,ip_acl_list,ip_acl_enabled,ip_whitelist,ip_whitelist_enabled,ip_blacklist,rate_limit_enabled,rate_limit_rps,rate_limit_burst,crs_rule_groups,crs_excluded_rules,custom_rules,block_page_id,block_status_code,enabled,updated_by,created_at,updated_at,geoip_countries,geoip_mode,waf_check_response,log_request_body,ip_acl_list_refs,ip_whitelist_refs,policy_type,trust_detection) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			...
			snapshotJSONText(p["ip_acl_list_refs"]), snapshotJSONText(p["ip_whitelist_refs"]), InferSnapshotPolicyType(p), snapshotTrustDetection(p)); err != nil {
...
func applySecurityBlockPages(ctx context.Context, tx *sql.Tx, pages []models.SecurityBlockPage) error {
	for _, page := range pages {
		if _, err := tx.ExecContext(ctx, `INSERT INTO security_block_pages (id,name,description,content,content_type,is_default,is_builtin,created_by,created_at,updated_by,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
			page.ID, page.Name, page.Description, page.Content, page.ContentType, page.IsDefault, page.IsBuiltin, page.CreatedBy, page.CreatedAt, page.UpdatedBy, page.UpdatedAt); err != nil {
