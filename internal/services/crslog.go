...
func writeCRSUpdateLog(level, stage, message string) {
	...
	if info, err := os.Stat(path); err == nil && info.Size() >= getCertJobLogSizeBytes() {
		...
		if rerr := rotateCertJobLogFiles(path); rerr != nil {
			Logf("info", "crs update log: rotation failed (oldest generation may be lost): %v", rerr)
		}
	}
	if err := os.MkdirAll(crsUpdateLogDir, 0755); err != nil {
		Logf("info", "crs update log: failed to create dir: %v", err)
		return
	}
	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
	if err != nil {
		Logf("info", "crs update log: failed to open %s: %v", path, err)
		return
	}
	defer f.Close()
