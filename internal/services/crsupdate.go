...
func (m *CRSUpdateManager) run(trigger string) {
	...
	if tag == currentCRSVersion() {
		...
		if _, err := db.DB.Exec(
			"UPDATE security_crs_version SET update_status='success', message='已是最新版本', finished_at=datetime('now') WHERE id=1",
		); err != nil {
...
func (m *CRSUpdateManager) fail(cause error, restore bool) {
	...
	if _, err := db.DB.Exec(
		"UPDATE security_crs_version SET update_status='failed', message=?, finished_at=datetime('now') WHERE id=1",
		cause.Error(),
	); err != nil {
