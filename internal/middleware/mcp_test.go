...
func TestMCPEndpointAuthenticationGatesAndProtocol(t *testing.T) {
	...
	// 121 = 全量工具数（export_config 已恢复：HTTP 层对只读 Key 403、只读可见性对其隐藏；GET 工具随版本递增，与 server_test 的只读 53 同步维护）
	if len(payload.Result.Tools) != 121 {
		t.Fatalf("tool count=%d, want 121", len(payload.Result.Tools))
	}
}
